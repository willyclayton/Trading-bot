...
class Market:
    def __init__(self, seed: int, start: date = date(2026, 1, 2), warmup: int = 60):
        ...
        self._dates = trading_dates(start, 4000)
        for _ in range(warmup):
            self.advance()

    @property
    def day(self) -> int:
        return len(next(iter(self.bars.values()))) - 1
    ...
    def advance(self) -> int:
        ...
        for sym, start_px, vol, drift in UNIVERSE:
            ...
            self.bars[sym].append(
                Bar(day, self._dates[day].isoformat(), round(open_, 4), round(high, 4),
                    round(low, 4), round(close, 4), vol_shares)
            )
        return day
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "bars": {s: [list(b.__dict__.values()) for b in bs] for s, bs in self.bars.items()},
        }
    ...
    def from_dict(cls, d: dict) -> "Market":
        ...
        m._dates = trading_dates(m.start, 4000)
